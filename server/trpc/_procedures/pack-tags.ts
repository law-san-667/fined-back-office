...
export const packTagsRouter = createTRPCRouter({
  getTags: privateProcedure.query(async ({ ctx }) => {
    ...
    if (!data) {
      if (shouldLog) console.error("No data found in pack_tags");
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
  ...
  createTag: privateProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { error: insertError } = await (supabase as any).from("pack_tags").insert({
        name,
        slug,
      });
