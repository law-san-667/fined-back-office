...
import { NewsTagsToolbar } from "./news-tags-toolbar";

interface NewsTagsTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
  client?: boolean;
}

export function NewsTagsTable<TData, TValue>({
  columns,
  data,
  client,
}: NewsTagsTableProps<TData, TValue>) {
  const [openDelete, setOpenDelete] = React.useState(false);

  const utils = trpc.useUtils();

  const { mutate: deleteSelectedTags, isPending: isDeleting } =
    trpc.newsTags.deleteSelectedTags.useMutation({
      onSuccess: () => {
