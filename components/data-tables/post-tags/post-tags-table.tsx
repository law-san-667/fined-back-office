...
import { PostTagsToolbar } from "./post-tags-toolbar";

interface PostTagsTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
  client?: boolean;
}

export function PostTagsTable<TData, TValue>({
  columns,
  data,
  client,
}: PostTagsTableProps<TData, TValue>) {
  const [openDelete, setOpenDelete] = React.useState(false);

  const utils = trpc.useUtils();

  const { mutate: deleteSelectedTags, isPending: isDeleting } =
    trpc.postTags.deleteSelectedTags.useMutation({
      onSuccess: () => {
