...
import { PackTagsToolbar } from "./pack-tags-toolbar";

interface PackTagsTableProps<TData, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
  client?: boolean;
}

export function PackTagsTable<TData, TValue>({
  columns,
  data,
  client,
}: PackTagsTableProps<TData, TValue>) {
  const [openDelete, setOpenDelete] = React.useState(false);

  const utils = trpc.useUtils();

  const { mutate: deleteSelectedTags, isPending: isDeleting } =
    trpc.packTags.deleteSelectedTags.useMutation({
      onSuccess: () => {
        toast.success("Les tags ont été supprimés avec succès");
